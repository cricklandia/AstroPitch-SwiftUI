...
//    let zeroFrequency = getFloatForKeyOrDefault(OptionNames.frequency, 0)
    let frequency = getFloatForKeyOrDefault(OptionNames.frequency, 262)
    let frequency2 = getFloatForKeyOrDefault(OptionNames.frequency, 311)
    let frequency3 = getFloatForKeyOrDefault(OptionNames.frequency, 392)
    ...
    var currentPhase2: Float = 0
    var currentPhase3: Float = 0
    // The interval to advance the phase each frame.
    let phaseIncrement = (twoPi / sampleRate) * frequency
    let phaseIncrement2 = (twoPi / sampleRate) * frequency2
    let phaseIncrement3 = (twoPi / sampleRate) * frequency3
    ...
    let srcNode2 = AVAudioSourceNode { _, _, frameCount, audioBufferList -> OSStatus in
        let ablPointer = UnsafeMutableAudioBufferListPointer(audioBufferList)
        for frame in 0..<Int(frameCount) {
            // Get the signal value for this frame at time.
            let value = signal(currentPhase2) * amplitude
            // Advance the phase for the next frame.
            currentPhase2 += phaseIncrement2
            if currentPhase2 >= twoPi {
                currentPhase2 -= twoPi
            }
            if currentPhase2 < 0.0 {
                currentPhase2 += twoPi
            }
            // Set the same value on all channels (due to the inputFormat, there's only one channel though).
            for buffer in ablPointer {
    ...
    let srcNode3 = AVAudioSourceNode { _, _, frameCount, audioBufferList -> OSStatus in
        let ablPointer = UnsafeMutableAudioBufferListPointer(audioBufferList)
        for frame in 0..<Int(frameCount) {
            // Get the signal value for this frame at time.
            let value = signal(currentPhase3) * amplitude
            // Advance the phase for the next frame.
            currentPhase3 += phaseIncrement3
            if currentPhase3 >= twoPi {
                currentPhase3 -= twoPi
            }
            if currentPhase3 < 0.0 {
                currentPhase3 += twoPi
            }
            // Set the same value on all channels (due to the inputFormat, there's only one channel though).
            for buffer in ablPointer {
...
//    engine.attach(srcNode2)
//    engine.attach(srcNode3)

    engine.connect(srcNode, to: mainMixer, format: inputFormat)
//    engine.connect(srcNode2, to: mainMixer, format: inputFormat)
//    engine.connect(srcNode3, to: mainMixer, format: inputFormat)
    ...
    if let path = outputPath {
        ...
        if outDirExists != nil {
            ...
            srcNode.installTap(onBus: 0, bufferSize: 1024, format: inputFormat) { buffer, _ in
                ...
                print("samplesWritten \(samplesWritten)")
    ...
    var outFile2: AVAudioFile?
    if let path2 = outputPath {
        var samplesWritten: AVAudioFrameCount = 0
        let outUrl = URL(fileURLWithPath: path2).standardizedFileURL
        let outDirExists = try? outUrl.deletingLastPathComponent().checkResourceIsReachable()
        if outDirExists != nil {
            var outputFormatSettings = srcNode2.outputFormat(forBus: 0).settings
            // Interleave the audio files.
            outputFormatSettings[AVLinearPCMIsNonInterleaved] = false
            outFile2 = try? AVAudioFile(forWriting: outUrl, settings: outputFormatSettings)
            // Calculate the total number of samples to write for the duration.
            let samplesToWrite = AVAudioFrameCount(duration * sampleRate)
            srcNode2.installTap(onBus: 0, bufferSize: 1024, format: inputFormat) { buffer, _ in
                // Check whether to adjust the buffer frame length to match
                // the requested number of samples.
                if samplesWritten + buffer.frameLength > samplesToWrite {
                    buffer.frameLength = samplesToWrite - samplesWritten
                ...
                do {
                    try outFile2?.write(from: buffer)
                } catch {
                    print("Error writing file \(error)")
                }
                samplesWritten += buffer.frameLength
    ...
    var outFile3: AVAudioFile?
    if let path3 = outputPath {
        var samplesWritten: AVAudioFrameCount = 0
        let outUrl = URL(fileURLWithPath: path3).standardizedFileURL
        let outDirExists = try? outUrl.deletingLastPathComponent().checkResourceIsReachable()
        if outDirExists != nil {
            var outputFormatSettings = srcNode3.outputFormat(forBus: 0).settings
            // Interleave the audio files.
            outputFormatSettings[AVLinearPCMIsNonInterleaved] = false
            outFile3 = try? AVAudioFile(forWriting: outUrl, settings: outputFormatSettings)
            // Calculate the total number of samples to write for the duration.
            let samplesToWrite = AVAudioFrameCount(duration * sampleRate)
            srcNode3.installTap(onBus: 0, bufferSize: 1024, format: inputFormat) { buffer, _ in
                // Check whether to adjust the buffer frame length to match
                // the requested number of samples.
                if samplesWritten + buffer.frameLength > samplesToWrite {
                    buffer.frameLength = samplesToWrite - samplesWritten
                ...
                do {
                    try outFile3?.write(from: buffer)
                } catch {
                    print("Error writing file \(error)")
                }
                samplesWritten += buffer.frameLength
